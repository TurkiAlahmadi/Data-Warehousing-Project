...
select cal_dt, d_date_sk from TPCDS.RAW.DATE_DIM order by 1 limit 100; -- d_date_sk increases with cal_dt

----- Catalog Sales
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select min(CS_SOLD_DATE_SK) from TPCDS.RAW.CATALOG_SALES); -- 2021-01-01
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select max(CS_SOLD_DATE_SK) from TPCDS.RAW.CATALOG_SALES); -- 2024-03-27


----- Web Sales
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select min(WS_SOLD_DATE_SK) from TPCDS.RAW.WEB_SALES); -- 2021-01-02
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select max(WS_SOLD_DATE_SK) from TPCDS.RAW.WEB_SALES); -- 2024-03-27

------ Inventory
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select min(INV_DATE) from TPCDS.RAW.INVENTORY); -- 2021-01-01
select cal_dt from TPCDS.RAW.DATE_DIM where d_date_sk = (select max(INV_DATE) from TPCDS.RAW.INVENTORY); -- 2024-03-26
...
group by 1,2
order by 1 desc
limit 1000;-- About 800 records everyday
...
group by 1,2
order by 1 desc
limit 100;-- About 300 records everyday
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.CUSTOMER_DEMOGRAPHICS as dim
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.HOUSEHOLD_DEMOGRAPHICS as dim
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.CUSTOMER_ADDRESS as dim
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.CALL_CENTER as dim
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.SHIP_MODE as dim
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.WAREHOUSE as dim
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.ITEM as dim
...
from 
    TPCDS.RAW.CATALOG_SALES as fact
INNER JOIN TPCDS.RAW.PROMOTION as dim
...
select count(distinct i_item_sk) from TPCDS.RAW.ITEM; -- 18000 items
